...
yum update -y
yum install -y wget curl >> /tmp/install-curl.log 2>&1
yum install -y initscripts >> /tmp/install-initscripts.log 2>&1
...
# yum install -y oracle-java8-installer>> /tmp/install-java8.log 2>&1

yum install -y java-1.8.0-openjdk >> /tmp/install-java8.log 2>&1
...
wget https://jfrog.bintray.com/artifactory-pro-rpms/jfrog-artifactory-pro/jfrog-artifactory-pro-${ARTIFACTORY_VERSION}.rpm -O /tmp/jfrog-artifactory-pro-${ARTIFACTORY_VERSION}.rpm
yum localinstall -y /tmp/jfrog-artifactory-pro-${ARTIFACTORY_VERSION}.rpm >> /tmp/install-artifactory.log 2>&1
rm -rf /tmp/jfrog-artifactory-pro-${ARTIFACTORY_VERSION}.rpm
...
curl -L -o  /opt/jfrog/artifactory/app/artifactory/tomcat/lib/mssql-jdbc-6.2.1.jre8.jar https://bintray.com/artifact/download/bintray/jcenter/com/microsoft/sqlserver/mssql-jdbc/6.2.1.jre8/mssql-jdbc-6.2.1.jre8.jar
